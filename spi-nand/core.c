...
static int spinand_init_cfg_cache(struct spinand_device *spinand)
{
	...
	spinand->cfg_cache =
		calloc(nand->memorg.ntargets, sizeof(*spinand->cfg_cache));
...
int spinand_ecc_enable(struct spinand_device *spinand,
			      bool enable)
{
			     ...
			     bool ecc_enabled)
{
	...
	ret = spinand_select_target(spinand, req->pos.target);
	if (ret)
		return ret;

	ret = spinand_ecc_enable(spinand, ecc_enabled);
	if (ret)
		return ret;
...
int spinand_erase(struct spinand_device *spinand, const struct nand_pos *pos)
{
	u8 status;
	int ret;
...
static int spinand_create_dirmaps(struct spinand_device *spinand)
{
	...
	spinand->dirmaps = calloc(1, sizeof(*spinand->dirmaps) *
...
static int spinand_detect(struct spinand_device *spinand)
{
	...
	if (ret) {
		fprintf(stderr, "unknown raw ID %hhX %hhX %hhX %hhX\n",
...
static int spinand_init(struct spinand_device *spinand)
{
	 ...
	 */
	spinand->scratchbuf = calloc(SPINAND_MAX_ID_LEN, 1);
	 ...
	 */
	spinand->databuf = malloc(nanddev_page_size(nand) +
			       nanddev_per_page_oobsize(nand));
	if (!spinand->databuf) {
		ret = -ENOMEM;
		goto err_free_bufs;
...
struct spinand_device _spinand;

struct spinand_device *spinand_probe(struct spi_mem *mem)
{
